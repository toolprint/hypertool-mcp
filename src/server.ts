...
async function startServer(config: SmitheryConfig = {}): Promise<void> {
  ...
  if (hasEnvConfig) {
    ...
    logger.info(
      ...
      getConfigSourceDescription(envConfig)
    );
    ...
    if (!validation.valid) {
      logger.warn(
        "Environment configuration validation issues:",
        validation.errors
      );
    }
  ...
  try {
    ...
    if (config.mcpServers && Object.keys(config.mcpServers).length > 0) {
      ...
      try {
        ...
        temporaryConfigFile = path.join(
          tempDir,
          `smithery-mcp-config-${Date.now()}.json`
        );
      ...
      logger.info(
        "No MCP configuration from Smithery, using file-based discovery"
      );
    ...
    const shutdown = async (signal?: string) => {
      ...
      if (temporaryConfigFile) {
        try {
          ...
          logger.debug(
            `Cleaned up temporary config file: ${temporaryConfigFile}`
          );
        } catch (error) {
    ...
    const initOptions: ServerInitOptions =
      MetaMCPServerFactory.createInitOptions({
