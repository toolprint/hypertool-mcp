...
describe("MetaMCPError", () => {
  ...
  test("should serialize to JSON properly", () => {
    const error = new ConnectionError("Test error", "test-server", true, {
      extra: "data",
    });
...
describe("Specific Error Types", () => {
  test("ConnectionError should have correct properties", () => {
    ...
    expect(error.getUserMessage()).toBe(
      "Cannot connect to server 'server1': Connection failed"
    );
  });
  ...
  test("ToolNotFoundError should provide helpful message", () => {
    ...
    expect(error.isRetryable).toBe(false);
    expect(error.getUserMessage()).toBe(
      "Tool 'missing-tool' is not available. Check your configuration or try a different tool."
    );
  });
  ...
  test("ConfigurationError should not be retryable", () => {
    ...
    expect(error.isRetryable).toBe(false);
    expect(error.getUserMessage()).toBe(
      "Configuration issue: Invalid config format"
    );
  });
});
...
describe("Error Messages", () => {
  test("should provide different messages for getUserMessage vs message", () => {
    ...
    expect(error.getUserMessage()).toBe(
      "Cannot connect to server 'test-server': ECONNREFUSED: Connection refused by server at localhost:3000"
    );
  });
  ...
  test("should handle special cases in user messages", () => {
    ...
    expect(toolError.getUserMessage()).toBe(
      "Tool 'calculator' error: Tool execution failed"
    );

    const routingError = new RoutingError("No route found", "unknown-tool");
    expect(routingError.getUserMessage()).toBe(
      "Request routing failed: No route found"
    );
  });
});
