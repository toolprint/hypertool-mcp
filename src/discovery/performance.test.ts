...
import { DiscoveredTool, MCPToolDefinition } from "./types";

describe("Tool Discovery Performance Tests", () => {
  // Performance thresholds (adjust based on requirements)
  const PERFORMANCE_THRESHOLDS = {
    DISCOVERY_TIME_MS: 5000,      // 5 seconds for 1000 tools
    SEARCH_TIME_MS: 100,          // 100ms for search in 1000 tools
    CACHE_ACCESS_TIME_MS: 50,     // 50ms for cache operations
    HASH_CALCULATION_TIME_MS: 500, // 500ms for hash calculation
    MEMORY_USAGE_MB: 100,         // 100MB memory usage
  };

  const generateMockTools = (count: number, serverName: string = "perf-server"): MCPToolDefinition[] => {
  ...
  const generateDiscoveredTools = (count: number, serverName: string = "perf-server"): DiscoveredTool[] => {
    const mcpTools = generateMockTools(count, serverName);
    return mcpTools.map((tool, i) => 
      ToolHashUtils.createHashedTool(tool, serverName, `${serverName}.${tool.name}`)
    );
  };
  ...
  describe("Tool Cache Performance", () => {
    ...
    it("should handle concurrent access efficiently", async () => {
      ...
      const insertPromises = tools.slice(0, toolCount / 2).map(tool => 
        cache.set(tool.namespacedName, tool)
      );
      
      const retrievePromises = tools.slice(toolCount / 2).map(tool => 
        cache.get(tool.namespacedName)
      );
    ...
    it("should handle server-based operations efficiently", async () => {
      ...
      for (let s = 0; s < serversCount; s++) {
        const serverTools = generateDiscoveredTools(toolsPerServer, `server_${s}`);
  ...
  describe("Stress Testing", () => {
    it("should handle rapid tool updates", async () => {
      ...
      for (let i = 0; i < iterations; i++) {
        const tools = generateDiscoveredTools(toolsPerIteration, `stress-server-${i}`);
    ...
    it("should handle concurrent operations under load", async () => {
      ...
      const operationPromises = Array.from({ length: concurrentOperations }, async (_, i) => {
        const tools = generateDiscoveredTools(toolsPerOperation, `concurrent-server-${i}`);
        ...
        const operations = tools.map(async (tool, j) => {
          if (j % 3 === 0) {
            ...
            // Search operation
            lookupManager.search({ name: tool.name.substring(0, 5), fuzzy: true });
          }
        });
