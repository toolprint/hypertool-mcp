 ...
 */
export function parseEnvDotNotation(envVars?: Record<string, string>): ParsedEnvConfig {
  ...
  for (const key of configKeys) {
    ...
    try {
      ...
      console.warn(`Failed to parse environment variable ${key}=${value}:`, error);
    }
  }
...
export function getConfigSourceDescription(config: ParsedEnvConfig): string {
  ...
  if (config.mcpServers && Object.keys(config.mcpServers).length > 0) {
    sources.push(`mcpServers (${Object.keys(config.mcpServers).length} servers)`);
  }
 ...
 */
export function validateParsedConfig(config: ParsedEnvConfig): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
  ...
  if (config.mcpServers) {
    for (const [serverName, serverConfig] of Object.entries(config.mcpServers)) {
      ...
      if (!serverConfig.type) {
        errors.push(`Server '${serverName}' is missing required 'type' property`);
      }
      
      if (!serverConfig.command && serverConfig.type === 'stdio') {
        errors.push(`Server '${serverName}' is missing required 'command' property for stdio transport`);
      }
    }
  }
  
  if (config.logLevel && !['trace', 'debug', 'info', 'warn', 'error', 'fatal'].includes(config.logLevel)) {
