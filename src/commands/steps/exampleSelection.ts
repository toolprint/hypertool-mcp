...
export class ExampleSelectionStep implements WizardStep {
  ...
  async run(state: WizardState): Promise<WizardState> {
    ...
    if (!examplesAvailable) {
      output.error(
        "Example configurations are not available. Using fresh start instead."
      );
      return {
        ...state,
    ...
    choices.push(new inquirer.Separator());
    choices.push(
      new inquirer.Separator("── Full Featured (Requires API Keys) ──")
    );
    ...
    if (selectedExample.requiresSecrets) {
      output.displaySpaceBuffer(1);
      output.warn(
        "⚠️  This configuration requires API keys to be fully functional."
      );
