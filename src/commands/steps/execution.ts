...
export class ExecutionStep implements WizardStep {
  ...
  async run(state: WizardState): Promise<WizardState> {
    ...
    if (state.dryRun) {
      output.success(theme.success('✅ Dry run complete! No changes were made.'));
    } else {
  ...
  private async savePerAppConfig(appId: string, config: any): Promise<void> {
    ...
    const basePath = process.env.HYPERTOOL_CONFIG_PATH || path.join(os.homedir(), '.toolprint', 'hypertool-mcp');
