...
export class ToolsetCreationStep implements WizardStep {
  ...
  async run(state: WizardState): Promise<WizardState> {
    ...
    output.displaySpaceBuffer(1);
    output.info(
      "Toolsets let you activate specific groups of tools for different tasks."
    );
    output.displaySpaceBuffer(1);

    // Generate suggested toolsets
    const suggestedToolsets =
      this.generateSuggestedToolsets(selectedServerNames);
  ...
  private generateSuggestedToolsets(
    serverNames: string[]
  ): ToolsetDefinition[] {
    const toolsets: ToolsetDefinition[] = [];

    // Development toolset
    const devTools = serverNames.filter(
      (name) =>
        name.includes("git") ||
    ...
    // DevOps toolset
    const devopsTools = serverNames.filter(
      (name) =>
    ...
    // Data/AI toolset
    const dataTools = serverNames.filter(
      (name) =>
        name.includes("python") ||
  ...
  private async createCustomToolset(
    availableServers: string[]
  ): Promise<ToolsetDefinition | null> {
    output.displaySpaceBuffer(1);
