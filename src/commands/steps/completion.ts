...
export class CompletionStep implements WizardStep {
  ...
  async run(state: WizardState): Promise<WizardState> {
    ...
    if (state.toolsets.length > 0) {
      ...
      output.info(
        `  • With toolset: ${theme.command(`hypertool-mcp --equip-toolset ${firstToolset.name}`)}`
      );
    }
    ...
    if (state.selectedApps.length > 0) {
      ...
      output.info(
        `  • Specific app: ${theme.command(`hypertool-mcp --linked-app ${firstApp}`)}`
      );
    }
    ...
    output.info(
      `  • View config: ${theme.command("hypertool-mcp config show")}`
    );
    output.info(`  • Add servers: ${theme.command("hypertool-mcp mcp add")}`);
    output.info(
      `  • Create toolsets: ${theme.command("hypertool-mcp config toolset create")}`
    );
    ...
    // Tips based on setup
    if (
      state.installationType === "standard" &&
      state.selectedApps.includes("claude-desktop")
    ) {
      output.info(
        theme.muted(
          "💡 Tip: Restart Claude Desktop to use the new configuration"
        )
      );
    }
    ...
    if (state.toolsets.length === 0) {
      output.info(
        theme.muted(
          "💡 Tip: Create toolsets to organize your tools by workflow"
        )
      );
    }
