...
export function createHealthCommand(): Command {
  return new Command("health")
    ...
    .option("--json", "Output health information in JSON format", false)
    .option(
      "--timeout <seconds>",
...
async function handleHealthCommand(options: any): Promise<void> {
  ...
  try {
    ...
    if (!status.running) {
      ...
      console.error("");
      console.error(
        semantic.messageError("❌ No service is currently running")
      );
      console.error("");
...
function displayHumanHealth(healthResult: any, status: any): void {
  ...
  if (healthResult.healthy) {
    ...
    console.log(
      theme.info(`  Checked At: ${healthResult.timestamp.toLocaleString()}`)
    );
  } else {
    ...
    console.log(theme.info(`  Response Time: ${healthResult.responseTime}ms`));
    console.log(
      theme.warning(`  HTTP Status: ${healthResult.status || "N/A"}`)
    );
    console.log(theme.info(`  Endpoint: ${healthResult.endpoint}`));
    console.log(theme.info(`  Profile: ${healthResult.profile}`));
    console.log(
    ...
    if (healthResult.status === 0) {
      console.log(
        theme.muted("  • Service may not be running or not accessible")
      );
      console.log(
      ...
      console.log(
        theme.muted("  • Service is running but has internal errors")
      );
      console.log(
        theme.muted("  • Check service logs: hypertool-mcp service logs")
      );
    } else if (healthResult.status === 404) {
      console.log(
        theme.muted("  • Health endpoint may not be configured correctly")
      );
      console.log(theme.muted("  • Expected endpoint: /health"));
    } else {
      console.log(
        theme.muted(`  • Unexpected HTTP status: ${healthResult.status}`)
      );
  ...
  console.log(
    theme.muted("  • Restart service: hypertool-mcp service restart")
  );
