...
export function createStartCommand(): Command {
  return new Command("start")
    ...
    .option(
      "--equip-toolset <name>",
      "Toolset to equip on startup"
    )
    .option(
      "--group <name>",
      "Server group to load"
    )
    .option(
      "--force",
  ...
  try {
    ...
    if (!options.force) {
      ...
      if (currentStatus.running) {
        spinner.fail();
        console.error("");
        console.error(semantic.messageError(`❌ Service is already running (${currentStatus.profile}, PID: ${currentStatus.pid})`));
        console.error(theme.info("   Use --force to start anyway, or stop the existing service first"));
        console.error("");
    ...
    console.log(theme.muted(`  • Test HTTP: curl http://${status.host || 'localhost'}:${status.port}/health`));
    ...
    if (errorMessage.includes("already running")) {
      ...
      console.error(theme.muted("   • Check status: hypertool-mcp service status"));
      console.error(theme.muted("   • Stop existing: hypertool-mcp service stop"));
      console.error(theme.muted("   • Force restart: hypertool-mcp service restart"));
