...
async function handleRestartCommand(options: any): Promise<void> {
  ...
  try {
    ...
    console.log(theme.info(`  HTTP: http://${newStatus.host || 'localhost'}:${newStatus.port}`));
    
    if (wasRunning) {
      console.log(theme.info(`  Previous PID: ${currentStatus.pid} (terminated)`));
    }
    ...
    console.log(theme.muted(`  • Test HTTP: curl http://${newStatus.host || 'localhost'}:${newStatus.port}/health`));
    ...
    console.error(theme.muted("   • Check service status: hypertool-mcp service status"));
