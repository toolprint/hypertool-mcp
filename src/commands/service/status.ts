...
async function handleStatusCommand(options: any): Promise<void> {
  ...
  try {
    ...
    if (status.running) {
      ...
      console.log(
        theme.info(
          `  HTTP: http://${status.host || "localhost"}:${status.port}`
        )
      );
      console.log(
        theme.info(
          `  Health Endpoint: http://${status.host || "localhost"}:${status.port}/health`
        )
      );
      ...
      try {
        ...
        if (response.status === 200) {
          ...
          healthSpinner.warn();
          console.log(
            theme.warning(`⚠️  Service returned status ${response.status}`)
          );
        }
      } catch (error) {
        healthSpinner.fail();
        console.log(
          theme.warning("🔴 Service is not responding to health checks")
        );
      }
      ...
      console.log(
        theme.muted(
          `  • Test endpoint: curl http://${status.host || "localhost"}:${status.port}/health`
        )
      );
    } else {
      ...
      console.log(
        theme.muted("  • Start service: hypertool-mcp service start")
      );
      console.log(
        theme.muted(
          "  • Start with profile: hypertool-mcp service start --profile production"
        )
      );
      console.log("");
      console.log(theme.label("Available Profiles:"));
      console.log(
        theme.muted("  • development - Local development with debug logging")
      );
      console.log(
        theme.muted(
