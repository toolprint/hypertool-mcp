...
export function createUnlinkCommand(): Command {
  ...
  unlink
    .description('Unlink applications from HyperTool and optionally restore previous configurations')
      ...
      try {
        ...
        if (options.app) {
          ...
          if (!appIds.includes(options.app)) {
            output.error(`❌ Application '${options.app}' not found or not supported.`);
        ...
        if (!options.noRestore) {
          ...
          if (backups.length > 0) {
              ...
              const { confirmRestore } = await inquirer.prompt([
                {
                  type: 'confirm',
                  name: 'confirmRestore',
                  message: 'Would you like to restore configurations from this backup?',
                  default: true
                }
              ]);
            ...
            output.warn('⚠️  No backups available. HyperTool will be removed without restoration.');
          }
        }

        // Show what will be done
        output.displaySubHeader('This will:');
        output.displayInstruction(`1. Remove HyperTool proxy from: ${selectedApps.map(id => apps[id].name).join(', ')}`);
        if (useBackup) {
          output.displayInstruction('2. Restore previous MCP configurations from backup');
          output.displayInstruction('3. If restored config contains HyperTool, remove only HyperTool entry');
        } else {
          output.displayInstruction('2. Leave MCP configuration empty (no restoration)');
        }
        output.displayInstruction(`${useBackup ? '4' : '3'}. Keep your backup files for future reference`);
        output.displaySpaceBuffer(1);
          ...
          const { confirm } = await inquirer.prompt([
            {
              type: 'confirm',
              name: 'confirm',
              message: semantic.messageError(`⚠️  Are you sure you want to unlink ${appNames} from HyperTool?`),
              default: false
            }
          ]);
        ...
        if (result.restoredWithHypertool && result.restoredWithHypertool.length > 0) {
          output.displaySpaceBuffer(1);
          output.warn('⚠️  The following applications had HyperTool in their backup:');
          for (const appId of result.restoredWithHypertool) {
            output.displayInstruction(`• ${apps[appId].name}: HyperTool entry removed, other servers preserved`);
          }
        }
        ...
        if (useBackup) {
          const backups = await configManager.listBackups();
          if (backups.length > 1) {
            output.displayInstruction('• Use "hypertool-mcp config restore" to restore older backups if needed');
          }
        }
        output.displayInstruction('• You can re-link applications at any time with "hypertool-mcp config link"');
