  ...
  describe("validation", () => {
    ...
    it("should reject stdio transport without command", () => {
      ...
      expect(result.errors).toContain(
        "Server \"test-server\" with type 'stdio' missing required field: command"
      );
    });
    ...
    it("should reject sse transport without url", () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors).toContain(
        "Server \"sse-server\" with type 'sse' missing required field: url"
      );
    });
    ...
    it("should warn about command field in http/sse transport", () => {
      ...
      expect(result.warnings).toContain(
        "Server \"mixed-server\" with type 'sse' should use 'url' instead of 'command'"
      );
    });
    ...
    it("should reject invalid transport type", () => {
      ...
      expect(result.valid).toBe(false);
      expect(result.errors).toContain(
        'Server "bad-server" has invalid type: invalid-type'
      );
    });
  });
