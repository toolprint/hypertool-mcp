...
async function testConfigManager() {
  ...
  try {
    ...
    if (needsMigration) {
      ...
      console.log(chalk.green(`✅ Migrated: ${migrationResult.migrated.join(', ') || 'None'}`));
      if (migrationResult.failed.length > 0) {
        console.log(chalk.red(`❌ Failed: ${migrationResult.failed.join(', ')}`));
      }
    }
    ...
    console.log(chalk.green(`✅ Imported from: ${importResult.imported.join(', ') || 'None'}`));
    ...
    if (backups.length > 0) {
      ...
      console.log(chalk.gray(`   Latest: ${new Date(latest.metadata.timestamp).toLocaleString()}`));
      console.log(chalk.gray(`   Servers: ${latest.metadata.total_servers}`));
      console.log(chalk.gray(`   Apps: ${Object.keys(latest.metadata.applications).join(', ')}`));
    }

    // Test deployment (dry run)
    console.log(chalk.yellow('\n6. Testing deployment (dry run)...'));
    console.log(chalk.gray('   Would deploy HyperTool configuration to all applications'));
