    ...
    .action(async (name, options) => {
      try {
        ...
        if (options.json) {
          ...
          // Basic info
          output.info(`${theme.label('Type:')} ${theme.value(server.config.type)}`);
          
          // Transport-specific info
          if (server.config.type === 'stdio') {
            output.info(`${theme.label('Command:')} ${theme.value(server.config.command || 'N/A')}`);
            if (server.config.args && server.config.args.length > 0) {
              output.info(`${theme.label('Arguments:')} ${theme.value(JSON.stringify(server.config.args))}`);
            }
          } else {
            output.info(`${theme.label('URL:')} ${theme.value(server.config.url || 'N/A')}`);
          }
          ...
          if (server.metadata) {
            ...
            output.info(`  ${theme.subtle('Source:')} ${theme.value(server.metadata.app)}`);
            output.info(`  ${theme.subtle('Imported:')} ${theme.value(server.metadata.importedAt)}`);
            if (server.metadata.addedManually) {
              output.info(`  ${theme.subtle('Added:')} ${theme.value('Manually')}`);
            }
          }
        }
