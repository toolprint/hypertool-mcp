      ...
      try {
        ...
        if (options.transport === "stdio") {
          ...
          if (!command) {
            ...
            output.info(
              `Usage: hypertool mcp add --transport ${options.transport} <name> <url>`
            );
            process.exit(1);
          }
        ...
        output.info(
          `  ${theme.subtle("Type:")} ${theme.value(serverConfig.type)}`
        );

        if (serverConfig.type === "stdio") {
          output.info(
            `  ${theme.subtle("Command:")} ${theme.value(serverConfig.command!)}`
          );
          if (serverConfig.args) {
            output.info(
              `  ${theme.subtle("Arguments:")} ${theme.value(serverConfig.args.join(" "))}`
            );
          }
        } else {
          output.info(
            `  ${theme.subtle("URL:")} ${theme.value(serverConfig.url!)}`
          );
        }

        if (serverConfig.env && Object.keys(serverConfig.env).length > 0) {
          output.info(
            `  ${theme.subtle("Environment:")} ${Object.keys(serverConfig.env).length} variable(s)`
          );
        }
        ...
        if (
          ...
          output.info(
            `  ${theme.subtle("Headers:")} ${Object.keys(serverConfig.headers).length} header(s)`
          );
        }
      } catch (error) {
