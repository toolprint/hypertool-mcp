...
describe('PersonaMcpIntegration', () => {
  ...
  let mockSetCurrentConfig: MockedFunction<(config: MCPConfig) => Promise<void>>;
  ...
  describe('applyPersonaConfig', () => {
    ...
    it('should handle environment variable merging', async () => {
      ...
      const mergedServer = result.mergedConfig!.mcpServers['base-server'] as any;
    ...
    it('should handle connection restart failures gracefully', async () => {
      mockGetCurrentConfig.mockResolvedValue(null);
      mockRestartConnections.mockRejectedValue(new Error('Connection restart failed'));

      const result = await integration.applyPersonaConfig(testConfigPath);

      expect(result.success).toBe(true); // Should still succeed
      expect(result.warnings).toContain('Connection restart failed: Connection restart failed');
    });
  });
  ...
  describe('restoreOriginalConfig', () => {
    ...
    it('should handle missing backup gracefully', async () => {
      ...
      await expect(integration.restoreOriginalConfig()).resolves.toBeUndefined();
      expect(mockSetCurrentConfig).not.toHaveBeenCalled();
    });
    ...
    it('should handle restoration errors', async () => {
      ...
      mockSetCurrentConfig.mockRejectedValue(new Error('Config restore failed'));

      await expect(integration.restoreOriginalConfig()).rejects.toThrow('Failed to restore original MCP config');
    });
  ...
  describe('configuration merging', () => {
    ...
    it('should handle base-wins conflict resolution', async () => {
      ...
      expect(result.warnings).toContain('Using base config for server "base-server"');
    });
    ...
    it('should handle user-choice conflict resolution', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.warnings.some(w => w.includes('Auto-resolved conflict'))).toBe(true);
    });
    ...
    it('should handle error conflict resolution', async () => {
      ...
      expect(result.success).toBe(false);
      expect(result.errors).toContain('Configuration conflict for server "base-server" - resolution required');
    });
  ...
  describe('validateMcpConfig', () => {
    it('should validate correct MCP config structure', () => {
      expect(PersonaMcpIntegration.validateMcpConfig(sampleBaseConfig)).toBe(true);
    });
    ...
    it('should reject invalid config structures', () => {
      ...
      expect(PersonaMcpIntegration.validateMcpConfig({ mcpServers: 'invalid' })).toBe(false);
    });
  });
});
...
describe('merge scenarios', () => {
  ...
  it('should handle complex merging scenario with multiple conflicts', async () => {
    ...
    expect(merged.mcpServers['server-b']).toEqual(complexBase.mcpServers['server-b']);
    expect(merged.mcpServers['server-c']).toEqual(complexBase.mcpServers['server-c']);

    // Check added persona server
    expect(merged.mcpServers['server-d']).toEqual(complexPersona.mcpServers['server-d']);
  });
});