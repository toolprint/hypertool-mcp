...
import { IDatabaseService, IServerConfigRecordRepository, IServerConfigGroupRepository } from './interfaces.js';
...
export class NeDBService implements IDatabaseService {
  ...
  async init(): Promise<void> {
    ...
    this._groups = new NeDBServerConfigGroupRepository(this.groupsDb, this._servers);
  ...
  private async createIndexes(): Promise<void> {
    return new Promise((resolve, reject) => {
      let completed = 0;
      const total = 4; // Total number of indexes to create
